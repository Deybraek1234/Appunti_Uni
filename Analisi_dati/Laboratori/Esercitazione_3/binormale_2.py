...
def initialize():
    ...
    def variance(x):
        ...
        return (1/(len(x)-1) * np.sum((x-media)**2))
    
    # genera errore sistematico
    S = np.random.normal(loc=0, scale=std_dev_s, size = n)
    # genera dati, global così anche la funzione plot_graphs può vederla
    global l1 
    l1 = np.random.normal(loc = mu1, scale = std_dev, size = n) + S
    global l2 
    l2 = np.random.normal(loc = mu2, scale = std_dev, size = n) + S

    # histogramma normale prima misura, questa volta si usano varianze calcolate dai dati
    counts, borders, _ = axs[0,0].hist(l1, bins = 80, range=[mu1-4*np.sqrt(variance(l1)), mu1+4*np.sqrt(variance(l1))], edgecolor = 'k', label="Valori generati mu1")
    ...
    expected_centers = gaussian_func(centers, mu1, np.sqrt(variance(l1))) * n * bin_width
    ...
    counts, borders, _ = axs[0,1].hist(l2, bins = 80, range=[mu2-4*np.sqrt(variance(l2)), mu2+4*np.sqrt(variance(l2))], edgecolor = 'k', label="Valori generati mu2")
    axs[0,1].set_xlabel("Valori generati centrati in 0.02")
    axs[0,1].set_xlabel("Frequenza")
    # calcoli per gaussiana seconda
    centers = (borders[:-1] + borders[1:])/2
    bin_width = borders[1] - borders[0]
    expected_centers = gaussian_func(centers, mu2, np.sqrt(variance(l2))) * n * bin_width 
    ...
    print(f"Varianza l1: {variance(l1)} \nVarianza l2: {variance(l2)} \nVarianza S: {variance(S)}")
...
def calcolo_cov():
    ...
    print(f"R: {r}")
    valori_q2 = q2(r, l1, l2, mu1, mu2, np.std(l1), np.std(l2)) 
    ...
    # plottare i punti
    axs[1,0].scatter(l1, l2, label=f"Tutti punti: {len(l1)}", s = 1)
    axs[1,0].scatter(punti_interni_1, punti_interni_2, label=f"Punti Interni: {len(punti_interni_1)}\n Rapporto: {len(punti_interni_1)/n*100}%", s =1)
    axs[1,0].axis('equal')
    ...
    centers = (borders[:-1] + borders[1:])/2
    bin_width = borders[1] - borders[0]
    centers_expected = gaussian_func(centers, mu1+r*(np.std(l1)/np.std(l2))*(valore_fissato-0.02), np.std(l1)*np.sqrt(1-r**2)) * len(punti_fascia) * bin_width
    errors = np.sqrt(counts)
    ...
    axs[1,1].plot(centers, gaussian_func(centers, mu1, std_dev) * bin_width * len(punti_fascia), label="Curva mu1 e dev_strd normali")
    axs[1,1].legend()
