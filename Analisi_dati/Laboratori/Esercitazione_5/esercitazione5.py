...
fig, a = plot.subplots(nrows=2, ncols=3)
axs = a.flatten()
data = np.loadtxt("gruppo2.txt")/1000
...
def Erlang(dati):
    ...
    axs[2].axhline(chisq_min, c = 'm', label=f"Lamb_min: {lamb_min}")
...
def Poisson(dati):
    ...
    axs[5].axhline(chisq_min, c='m', label=f"Lamb_min: {lamb_min}")
