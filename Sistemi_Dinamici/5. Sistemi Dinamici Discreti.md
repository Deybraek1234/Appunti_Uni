...
>$$
> d(s,t) = \sum_{i=0}^{\infty} \frac{\left.\vert s_{i} - t_{i}\right.\vert}{2}
>$$
>in questo modo ho che la mia distanza è limitato superiormente da 
>$$
> d(s,t) \leq \sum_{i=0}^{\infty} \frac{1}{2^i} = \frac{1}{1-\frac{1}{2}} = 2
>$$
>dove si è usato il fatto che la serie fosse geometrica
...
>[!thm] 
>La funzione $\begin{align}S:  & \Lambda \rightarrow \Sigma \\  & S \mapsto (s_{0}, s_{1}, \dots)\end{align}$ itinerario è un omeomorfismo. 
